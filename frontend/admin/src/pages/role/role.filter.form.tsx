...
import { useFilterForm } from "@/pages/role/role.filter.form.schema.tsx";
import { valuesResolver } from "@/pages/role/role.filter.form.schema.tsx";
import { RoleQuery } from "@/sys/role/role.types.ts";

export const RoleFilterForm: FC = () => {
  const { t } = useTranslation();

  const { query, setQuery } = useTableData<RoleQuery>();
        ...
        <form onSubmit={form.handleSubmit(submit)} className="flex flex-col xl:flex-row gap-4">
                ...
                <FormControl>
                  <div className="relative w-full max-w-sm">
                    <Input
                      ...
                      placeholder={t("role.filter.nickname.placeholder")}
                      {...field}
                    />
                    <Button
                      type="button"
          ...
          <FormField
            ...
            render={() => (
              <FormItem>
                <OptionSelector
                  options={enumToOptions(RoleStatus, "role.status")}
                  placeholder={t("role.status.unselected")}
                  className="h-8"
